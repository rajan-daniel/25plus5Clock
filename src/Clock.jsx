...
export const Clock = () => {
    ...
    const checkTimeEnd = useRef(false);
    ...
    return (
        <>  
            <BreakTimeControl breakMinutes={breakMinutes} setbreakMinutes={setbreakMinutes} setTime={setTime} isRunning={running} />
...
export const TimeControl = ({ minutes, setMinutes, setTime, isRunning }) => {
    return (
        <>
            ...
            <button onClick={() => { if (isRunning.current === false && minutes > 1) { setTime((minutes - 1) * 60); setMinutes((prev) => prev - 1); } }}>Decrement</button>
        </>
    );
}

export const BreakTimeControl = ({ breakMinutes, setbreakMinutes, isRunning }) => {
    return (
        <>
            <h1>Break Length</h1>
            <h1>{breakMinutes}</h1>
            <button onClick={() => { if (isRunning.current === false) { setbreakMinutes((prev) => prev + 1); } }}>Increment</button>
            <button onClick={() => { if (isRunning.current === false && breakMinutes > 1) { setbreakMinutes((prev) => prev - 1); } }}>Decrement</button>
        </>
    );
}